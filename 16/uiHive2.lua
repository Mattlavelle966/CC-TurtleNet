...
-- Uses ui_lib.lua for drawing + touch dispatch. :contentReference[oaicite:0]{index=0}
--
-- Fixes included:
-- 1) GOTO numbers are visible (no button redraw overwriting the text)
--    - We register "invisible hitboxes" without drawing over the input boxes.
-- 2) Fuel no longer shows nil
--    - We accept fuel from multiple possible keys (fuel, fuelLevel, fuel_count, etc.)
--    - Telemetry view uses the same getter (so it matches tablet behavior).
--
-- Zero Node changes required. Packet format: "id:<n> <cmd>"
...
local function ensureNodes(n)
	...
	for i = 1, nodeCount do
		if not nodes[i] then
			nodes[i] = { id = i, last = nil, lastRxAt = nil, lastRaw = nil, startY = nil }
		end
	end
end
...
local function parsePayload(raw)
	-- Some setups send strings; some send already-serialized tables; handle both.
...
local function getState(n)
	...
	return tostring(n.last.state or n.last.Status or "unknown")
end
...
local function getPos(n)
	...
	if type(n.last.pos) == "table" then
		return n.last.pos
	end
	-- fallback alternate names if you ever use them
	if type(n.last.position) == "table" then
		return n.last.position
	end
	return nil
end
...
local function getFuel(n)
	-- You said fuel is solid on tablet. That usually means the key name differs.
...
-- ========= INVISIBLE HITBOX (no redraw) =========
local function hitbox(id, x, y, w, h, onClick)
...
local function drawDetail()
	...
	if n.last and type(n.last) == "table" then
		...
		UI.drawText(
			3,
			6,
			("id:%s facing:%s fuel:%s"):format(tostring(n.last.id), tostring(n.last.facing), fuel),
			colors.black,
			colors.gray
		)
		UI.drawText(
	...
	local displayY = (yMode == "start") and (effectiveStartY() and tostring(effectiveStartY()) or "?")
		or (yCustom == "" and " " or yCustom)
...
local function bumpField(field, delta)
	...
	if v == nil then
		local n = nodes[selectedId]
		local p = (n and n.last and getPos(n)) or nil
...
local function drawGoto()
	...
	UI.drawText(2, 2, "Tap X/Y/Z to type. Use +/- to nudge. STEP cycles 1/5/10/50.", colors.gray, colors.black)
	...
	-- IMPORTANT: No UI.drawButton over the input box (it overwrites text).
	-- We draw the box+text, then register an invisible hitbox.
	local function fieldRow(label, field, rowY)
		...
		local txt = gotoEntry[field]
		if txt == "" then
			txt = " "
		end
		UI.drawText(7, rowY, txt, colors.black, boxBg)

		-- invisible click area (does not redraw the box)
		hitbox("hit_" .. field, 6, rowY, 10, 1, function()
			gotoField = field
			gotoEntryMode = true
		end)
...
local function drawSettings()
	...
	UI.drawText(2, 12, "Fuel is read from multiple keys (tablet-compatible).", colors.gray, colors.black)
	...
	elseif e[1] == "modem_message" and e[3] == CMD_SEND_CHANNEL then
		...
		if t and type(t) == "table" then
			...
			if id and id >= 1 then
				...
				local p = getPos(n)
				if not n.startY and p and p.y ~= nil then
					n.startY = tonumber(p.y)
				end

				log(("RX id:%d cmd:%s state:%s fuel:%s"):format(id, tostring(t.cmd), tostring(t.state), getFuel(n)))
			else
