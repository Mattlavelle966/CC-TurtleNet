...
function MineNetUI.ClientConnector(color)
	UI.drawGrid(clientX + boxW, clientY + math.floor(boxH / 2), 4, 1, 1, 1, function()
		return color
	end)
end
function MineNetUI.NodesConnector(color)
	UI.drawGrid(nodesX + boxW, nodesY + math.floor(boxH / 2), 4, 1, 1, 1, function()
		return color
	end)
end
function MineNetUI.CollectorConnector(color)
	UI.drawGrid(nodesX - 11, nodesY + boxH + 3, 1, 1, 1, 1, function()
		return color
	end)
	UI.drawGrid(nodesX - 11, nodesY + boxH + 2, 41, 1, 1, 1, function()
		return color
	end)
	UI.drawGrid(packetColX + boxW + 2, packetColY + boxH, 1, 3, 1, 1, function()
		return color
	end)
end
function MineNetUI.ManipConnector(color)
	UI.drawGrid(packetManipX + boxW + 2, packetManipY + math.floor(boxH / 2), 6, 1, 1, 1, function()
		return color
	end)
end
